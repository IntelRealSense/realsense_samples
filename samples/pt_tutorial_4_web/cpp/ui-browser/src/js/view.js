...
transporter.onPTDataUpdate = (timestamp, pt_data) => {
	...
	this.prev_pt_result_list = pt_result_list;	// Remember for click on canvas
        ...
        for (var i = 0; i < pt_result_list.length; i++) {
            ...
            var data_final = getCurrentTime(timestamp) + ";" + "pid: " + pt_object.pid;
            console.log("pid is: " + pt_object.pid); 
            ...
            if(pt_object.hasOwnProperty('person_bounding_box'))
            {
                ...
                if(pt_object.hasOwnProperty('rid'))
                {
                    ctx2d.fillText("rid: " + pt_object.rid, rectX+1, rectY+10);
                }
                else {
...
{
    console.log("< < < < < < < < < < < < < < < < < < < < onOverlayCanvasClick > > > > > > > > > > > > > > > > > >");
    var element = overlayCanvas;
    ...
    // for any person we currently know about (by looking at the prev_pt_result_list)
    for (var i = 0; i < prev_pt_result_list.length; i++)
    {
        var pt_object = prev_pt_result_list[i];
        ...
        if (pt_object.hasOwnProperty('person_bounding_box'))
        {
            ...
            var rectW = pt_object.person_bounding_box.w * scaleFactor;
            var rectH = pt_object.person_bounding_box.h * scaleFactor;
            console.log("Person " + i + ":  l, r, t, b = " + rectX + ", " + (rectX+rectW) + ", " + rectY + ", " + (rectY+rectH));

            if (x >= rectX && x < rectX+rectW && y >= rectY && y < rectY+rectH)	// If click is in a person's rect
            {
                console.log("Click was in person " + i);
                if (pt_object.hasOwnProperty('pid'))                            // and if we have ID for the person
                {
